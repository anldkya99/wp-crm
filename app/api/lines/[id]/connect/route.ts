...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const result = await connectionEngine.start(id, operatorId);
    const payload = await linePayload(id);
    return NextResponse.json({ ...payload, result });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "WhatsApp bağlantısı başlatılamadı." }, { status: 500 });
  }
}
